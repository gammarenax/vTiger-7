    ...
    <thead>
        <tr class="blockHeader">
            <th class="mediumWidthType">
                <span class="alignMiddle">{vtranslate('LBL_COMPANY_LICENSE_INFO', 'Settings:ITS4YouDynamicFields')}</span>
            </th>
            <th class="mediumWidthType" style="border-left: none; text-align: right;">
                <button type="button" id="company_button" class="btn btn-info" onclick="window.location.href='index.php?module=Vtiger&parent=Settings&view=CompanyDetails&block=3&fieldid=14'"/>{vtranslate('LBL_CHANGE_COMPANY_INFORMATION','Settings:ITS4YouDynamicFields')}</button>
            </th>
        </tr>
    </thead>
    <tbody>
...
{if $STEP eq ""}<label class="fieldLabel"><strong>{vtranslate('LBL_LICENSE_SETTINGS_INFO','Settings:ITS4YouDynamicFields')}:</strong></label><br>{/if}
<table class="table table-bordered table-condensed themeTableColor">
    <thead>
            <tr class="blockHeader">
                    <th colspan="2" class="mediumWidthType">
                            <span class="alignMiddle">{vtranslate('LBL_LICENSE', 'Settings:ITS4YouDynamicFields')}</span>
                    </th>
            </tr>
    </thead>
    <tbody>
            <tr>
                <td width="25%"><label  class="muted pull-right marginRight10px">{vtranslate('LBL_URL', 'Settings:ITS4YouDynamicFields')}:</label></td>
                <td style="border-left: none;">
                    <div class="pull-left" id="vatid_label">{$URL}</div>
                </td>
            </tr>
            <tr class="license_due_date_tr {if $LICENSE_DUE_DATE eq "" || $LICENSE eq ""}hide{/if}">
            ...
            <tr>
                <td width="25%"><label  class="muted pull-right marginRight10px">{vtranslate('LBL_LICENSE_KEY', 'Settings:ITS4YouDynamicFields')}:</label></td>
                <td style="border-left: none;">
                    
                    {if $STEP neq ""}
                        <input type="text" class="input-large" id="licensekey" name="licensekey" data-validation-engine="validate[required]"/>
                        <button type="subbmit" id="validate_button" class="btn btn-success"/><strong>{vtranslate('LBL_VALIDATE','Settings:ITS4YouDynamicFields')}</strong></button>&nbsp;&nbsp;
                        <button type="button" id="order_button" class="btn btn-info" onclick="window.location.href='http://www.its4you.sk/en/vtiger-shop.html'"/>{vtranslate('LBL_ORDER_NOW','Settings:ITS4YouDynamicFields')}</button>
                    {else}
                        ...
                        <div id="divgroup1" class="btn-group pull-left paddingLeft10px" {if $VERSION_TYPE eq "basic" || $VERSION_TYPE eq "professional"}style="display:none"{/if}>
                            <button id="activate_license_btn"  class="btn addButton" title="{vtranslate('LBL_ACTIVATE_KEY_TITLE','Settings:ITS4YouDynamicFields')}" type="button"><strong>{vtranslate('LBL_ACTIVATE_KEY','Settings:ITS4YouDynamicFields')}</strong></button>
                        </div>
                        <div id="divgroup2" class="pull-left paddingLeft10px" {if $VERSION_TYPE neq "basic" && $VERSION_TYPE neq "professional"}style="display:none"{/if}>
                            <button id="reactivate_license_btn"  class="btn btn-success" title="{vtranslate('LBL_REACTIVATE_DESC','Settings:ITS4YouDynamicFields')}" type="button">{vtranslate('LBL_REACTIVATE','Settings:ITS4YouDynamicFields')}</button>
                            <button id="deactivate_license_btn" type="button" class="btn btn-danger marginLeftZero">{vtranslate('LBL_DEACTIVATE','Settings:ITS4YouDynamicFields')}</button>
                        </div>
                    {/if}
                </td>
            </tr>
